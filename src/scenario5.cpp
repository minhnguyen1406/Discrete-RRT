...
bool isGroupStateCollisionFree(const ob::State *groupState, const vector<Rectangle> & barriers) {
    ...
    for (int robotIndex = 0; robotIndex < 6; ++robotIndex) {
...
void plan(og::SimpleSetup & setup, const PRMptr& PRMplanner, int planner)
{
    ...
    switch (planner) {
        case 1:
            ...
            break;
        case 2:
            setup.setPlanner(make_shared<og::DRRT>(setup.getSpaceInformation(), 6, PRMplanner));
            break;
    }
...
void benchmark(og::SimpleSetup setup, const PRMptr& PRMplanner)
{
    ...
    b.addPlanner(make_shared<og::DRRT>(setup.getSpaceInformation(), 6, PRMplanner));
